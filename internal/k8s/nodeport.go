...
func NewNodePortDiscovery(projectID string) (*NodePortDiscovery, error) {
	...
	config, err := buildK8sConfig(ctx, containerSvc, projectID)
	if err != nil {
...
func buildK8sConfig(ctx context.Context, containerSvc *container.Service, projectID string) (*rest.Config, error) {
...
func (d *NodePortDiscovery) DiscoverNodePorts(ctx context.Context) ([]int, error) {
	...
	slog.Info("NodePort discovery completed", "total_ports", len(ports), "ports", ports)
	return ports, nil
}
